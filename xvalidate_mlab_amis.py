...
from dist_sim.fcomb import FCombo, make_multilabel
...
learners=[AMISLearner('fold-%d'%(i,)) for i in xrange(n_bins)]
...
for bin_nr,data,label in all_data:
    ...
    n1=make_multilabel(wanted[0],unwanted,mkdata(data),fc)
    n1.assign_ids(0)
    n1.calcInsideMin(defaultdict(bool))
    n2=n1.extractBest(fc)
    for i in xrange(n_bins):
        if i!=bin_nr:
            f=streams[i]
            f.write('line_%s\n1\t'%(lineno,))
            n2.write_amis(None,False,None,f,fc)
            f.write('\n')
            n1.write_amis(None,False,defaultdict(bool),f,fc)
            f.write('\n\n')
