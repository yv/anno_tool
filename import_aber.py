...
for t in export.read_trees(file(FNAME)):
    ...
    for n in t.terminals:
        if (n.word.lower()=='aber' and
            n.cat=='KON' and n.parent and
            n.parent.cat not in ['KOORD','ADVX','ADJX','NX']):
            c=n.word.lower()
            examples.append((t,t_last,n))
            if len(examples)==MAX_LEN:
                print packet_num
                write_mmax(examples,'aber_%02d'%(packet_num,))
                write_html(examples,'aber_%02d'%(packet_num,))
                examples=[]
                packet_num +=1
    t_last=t

if examples:
        write_mmax(examples,'aber_%02dx'%(packet_num,))
        write_html(examples,'aber_%02dx'%(packet_num,))
        packet_num +=1
