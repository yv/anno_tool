...
class MarkableAttribute(QueryBase):
    ...
    def __call__(self,markable):
        val=markable[2].get(self.name,None)
        return val
...
<body>"""
    for docid in anno_sets[which_set]:
        docs=[MMAXDiscourse(annodirs[anno],docid) for anno in names]
        mss=markable_join(docs)
        out=StringIO()
        for part in mss:
            if q(part):
                display_markable(part,names,docs[0],out)
        yield out.getvalue()
    yield "</body></html>"
